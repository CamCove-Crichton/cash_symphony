...
def get_remaining_budget(user_id, month, year):
    ...
    :rtype: str
    """
    profile = UserProfile.objects.filter(username__id=user_id)[0]
    year_month = tuple(year, month)
    expenses = get_outgoing_expense(year_month)
    # get budget from UserProfile
    # Use 'monthly_budget_limit' if not none
    # or use 'salary.'  If both are none, return None.
    if profile.monthly_budget_limit == None:
        if profile.salary == None:
            return None
        budget = Decimal(profile.salary)
    else:
        budget = Decimal(profile.monthly_budget_limit)
    remaining_budget = Decimal(budget) - Decimal(expenses)
    remaining_budget_str = str("{:.2f}".format(remaining_budget))
    return remaining_budget_str
